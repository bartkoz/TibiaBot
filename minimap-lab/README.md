...
Krok prosty, po którym trzy kolejne świeże klatki pokazują postać wciąż na kratce startowej, tworzy **blokadę tymczasową** na 60 sekund. Drugi taki epizod awansuje kratkę na **blokadę trwałą**, zapisywaną w pliku wskazanym przez `-blocks` (domyślnie `blocks.json` w katalogu uruchomienia). Katalog map nie jest tu domyślną lokalizacją celowo: to pobrana paczka danych i nasze wpisy nie powinny się z nią mieszać ani ginąć przy jej podmianie.

Za drugi epizod uznaje się dopiero taki, który nastąpił po wygaśnięciu poprzedniej blokady **i po tym, jak postać w międzyczasie faktycznie gdzieś doszła**. Sam upływ czasu nie wystarcza: gracz stojący bez ruchu w drzwiach odbiłby bota, przeczekał minutę i odbił go znowu, a przejście zostałoby trwale wykreślone z mapy — trwała blokada nie przedawnia się sama. Bot, który obszedł ladę, wrócił i odbił się od niej ponownie, faktycznie zetknął się z nią dwa razy; bot stojący cały czas przed tym samym graczem — ani razu. Kolejne odbicie w trakcie życia blokady tylko przedłuża jej termin, nie liczy się jako nowy epizod.

Blokada tymczasowa **nie jest ścianą** — podnosi koszt kratki o 2000, czyli mniej więcej tyle, co dwadzieścia zwykłych kroków. Każde realne obejście lady jest tańsze i zostanie wybrane; objazd dłuższy niż to w praktyce znaczy, że obejścia nie ma. Gracz stojący w jednokratkowych drzwiach nie odcina wtedy trasy: jeśli objazd istnieje, zostanie wybrany, a jeśli nie — trasa nadal prowadzi tamtędy, bot czeka i próbuje ponownie, zamiast ogłosić brak drogi i stanąć. Dopiero blokada trwała czyni kratkę nieprzechodnią, tak samo jak 255 w danych mapy.

Wykonawca nie uczy się ze wszystkiego. Nieudany **skos** blokuje samo przejście na 20 sekund, nigdy kratkę: skos zawodzi także na zamkniętym rogu, gdzie oba kafle ortogonalne są zablokowane, a kratka docelowa bywa wtedy zupełnie pusta — nauka z takich prób stopniowo wycinałaby z mapy przechodnie skrzyżowania. Zmiana piętra nie uczy niczego, inaczej schody dostałyby blokadę: wejście na nie zmienia Z, więc kratka startowa i docelowa mają te same X i Y i krok wygląda na nieudany. Odmowa drivera, błąd połączenia i utrata pozycji też nie uczą — to problemy sterowania, nie mapy. Wejście na kratkę do 2 sekund po upływie timeoutu odwołuje naukę — to był lag albo paraliż — i cofa całą eskalację, którą ta porażka wywołała. Liczy się przy tym czas przechwycenia klatki, nie moment jej przetworzenia: wolne dopasowanie nie może zamienić prawdziwego wejścia w spóźnione.
...
Domyślne `battle_bar_tolerance` (80) i `battle_black_max` (48) to punkt startowy do kalibracji, tej samej rangi co domyślne barwy paska w akapicie wyżej — to nie opis żadnego przetestowanego klienta, tylko liczby do ręcznego dostrojenia na miejscu. Jedyny prawdziwy klient faktycznie sprawdzony w tym repo (`testdata/combat-capture.png`, kalibracja w `internal/testenv.CombatCalibration()`) potrzebował `BattleTolerance=60`, `BattleBlackMax=85` i listy barw przyciętej do własnych zmierzonych odcieni tego klienta — nie domyślnych 80/48 z domyślną listą barw. Zmierzony próg czerni battle listy (85) wyszedł przy tym **niższy** niż próg okna gry (125), nie wyższy: niewypełniona część paska na liście jest szara, nie czarna jak tło świata gry, ale to, w którą stronę i o ile trzeba przesunąć próg względem okna gry, zależy od konkretnego klienta i trzeba to sprawdzić na miejscu, a nie zakładać z góry.
