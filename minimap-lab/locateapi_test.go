...
func floorFixture(t testing.TB, oldZ int, targetFloors ...int) (*server, image.Image, matchRequest) {
	...
	s := &server{dir: dir, gate: make(chan struct{}, 1), cached: &mapdata.Atlas{Image: wrong, Origin: image.Pt(32768, 32000), Floor: oldZ}}
	for _, z := range targetFloors {
		testenv.SavePNG(t, filepath.Join(dir, fmt.Sprintf("Minimap_Color_32768_32000_%d.png", z)), ref)
	}
	req := matchRequest{Options: locate.Options{Zoom: 1, MarkerX: 52, MarkerY: 57, MaskRadius: 5, MinScore: .85, MinGap: .015},
		Floor: oldZ, Near: &mapdata.Position{X: 32957, Y: 32076, Z: oldZ}, Radius: 5, AdjacentFloors: true, FloorRadius: 8, NoPreview: true}
	return s, testenv.LoadFixture(t, "venore-capture.png"), req
}
