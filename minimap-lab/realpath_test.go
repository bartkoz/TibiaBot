...
func realCostGrid(t *testing.T, centre [2]int, radius int) *CostGrid {
	t.Helper()
	if os.Getenv("MINIMAP_REAL_MAP_TEST") != "1" {
		t.Skip("set MINIMAP_REAL_MAP_TEST=1 to test against the maps in this repository")
	}
	area := image.Rect(centre[0]-radius, centre[1]-radius, centre[0]+radius, centre[1]+radius)
	grid, err := loadCostArea("../data/minimap", 7, area)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRealMapRouteStaysOnWalkableGround(t *testing.T) {
	...
	for i, s := range r.Steps {
		if grid.At(s[0], s[1]) == blockedCost {
		...
		if dx != 0 && dy != 0 &&
			grid.At(previous[0]+dx, previous[1]) == blockedCost &&
			grid.At(previous[0], previous[1]+dy) == blockedCost {
...
func TestRealMapRefusesAWallAsAWaypoint(t *testing.T) {
	...
	for dy := -20; dy <= 20 && wall == [2]int{}; dy++ {
		for dx := -20; dx <= 20; dx++ {
			if grid.At(realEnclosed[0]+dx, realEnclosed[1]+dy) == blockedCost {
...
func TestRealMapReportsAnUnreachableWaypoint(t *testing.T) {
	...
	for dx := 10; dx <= 25 && outside == [2]int{}; dx++ {
		...
		if grid.At(candidate[0], candidate[1]) != blockedCost {
			outside = candidate
		}
	}
	if outside == [2]int{} {
