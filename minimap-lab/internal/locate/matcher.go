...
func WithScale(ctx context.Context, atlas *mapdata.Atlas, im image.Image, o Options) (Result, error) {
	if o.Zoom != 0 {
...
func locate(ctx context.Context, atlas *mapdata.Atlas, im image.Image, o Options) (Result, error) {
	return locateIn(ctx, atlas, im, o, nil)
}

// near and radius constrain PLAYER coordinates, not the top-left of the crop.
func Near(ctx context.Context, atlas *mapdata.Atlas, im image.Image, o Options, near mapdata.Position, radius int) (Result, error) {
...
func locateIn(ctx context.Context, atlas *mapdata.Atlas, im image.Image, o Options, worldArea *image.Rectangle) (Result, error) {
	...
	candidate := func(p image.Point, loss int) *Candidate {
		return &Candidate{mapdata.Position{X: atlas.Origin.X + p.X, Y: atlas.Origin.Y + p.Y, Z: atlas.Floor}, 1 - float64(loss)/denom}
	}
