...
import {Camera, REGION as FRAME_REGION, makeCut} from './camera.js';

export function createPanel(env) {
  const {document, localStorage, navigator, performance,
    Image, ImageData, Worker, Blob, FormData, URL} = env;
  // fetch and the timers are called bare throughout the panel. Pulled off
  // window by destructuring they would lose their receiver and throw Illegal
  // invocation, so they are wrapped rather than unpacked.
  const fetch = (...a) => env.fetch(...a);
  const setTimeout = (...a) => env.setTimeout(...a);
  const clearTimeout = (...a) => env.clearTimeout(...a);
  ...
  // stop puts the panel down: the browser entry never calls it, but a test
  // that builds one has to be able to release the shared screen and the loop.
  function stop() { stopShare(); }
